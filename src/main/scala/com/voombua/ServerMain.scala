...
import com.voombua.repos.{ DbConnector, ServiceDb }
import com.voombua.routes.MemberRoutes
...
object ServiceMain extends App with RequestTimeout {
  ...
  val db = new ServiceDb(DbConnector.default)

  val api = new MemberRoutes(new MemberCommands(db)).routes // the RestApi provides a Route
  ...
  val log = Logging(system.eventStream, "stack-overflow")
